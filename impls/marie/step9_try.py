...
def is_macro_call(ast: MALType, env: Env):
    match ast:
        case MALList([MALSymbol(symbol), *_]):
            try:
                value = env[symbol]
            except Exception:
                return False
            return isinstance(value, MALFunction) and value.is_macro
        case _:
            return False

def macroexpand(ast: MALType, env: Env):
    while is_macro_call(ast, env):
        symbol: MALSymbol = ast[0]
        args = ast[1:]
        macro = env[symbol.value]
        ast = macro(*args)
    return ast
...
def EVAL(ast: MALType, env: Env):
    while True:
        ...
        match ast:
            ...
            case MALList([MALSymbol("defmacro!"), MALSymbol(name), value]):
                v = EVAL(value, env)
                v.is_macro = True
                env[name] = v
                return v
            case MALList([MALSymbol("macroexpand"), value]):
                return macroexpand(value, env)
            case MALList([MALSymbol("try*"), func]): # bare try*
                return EVAL(func, env)
            case MALList([MALSymbol("try*"), func, MALList([MALSymbol("catch*"), error_symbol, otherwise])]):
                try:
                    return EVAL(func, env)
                except MALException as e:
                    new_env = Env(outer=env, binds=[error_symbol], exprs=[e.value])
                    return EVAL(otherwise, new_env)
